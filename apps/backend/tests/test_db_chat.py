...
from apps.backend.api.chat import (
    ChatRequest,
...
from apps.backend.core.fuzzy_match import rank_fuzzy_names
...
class DatabaseChatTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_bare_place_name_reretrieves_and_generates_as_a_question(self) -> None:
        ...
        with (
            ...
            patch(
                "apps.backend.api.chat.generate_response", return_value="Câu trả lời"
    ...
    async def test_location_without_database_candidate_does_not_reach_llm(self) -> None:
        with patch("apps.backend.api.chat.KgRepository") as repository_type:
    ...
    async def test_general_question_without_retrieved_evidence_does_not_reach_llm(self) -> None:
        with (
            patch("apps.backend.api.chat._try_db_location_answer", AsyncMock(return_value=None)),
            patch("apps.backend.api.chat.retrieve_context", return_value=("", [], [])),

...
if [[ $last_good == "$failed_revision" ]]; then
  echo 'Last-good revision equals the failed revision; rollback skipped' >&2
elif [[ $last_good =~ ^[0-9a-f]{40}$ ]]; then
  GIT_COMMIT=$last_good "$(dirname "$0")/deploy-staging.sh" || echo 'Rollback after deployment failure also failed' >&2
else

...
Chạy:
  python pipelines/training/bootstrap_deep_qa.py
  python pipelines/training/bootstrap_deep_qa.py --use-model    # dùng llama.cpp server làm teacher
"""
...
from apps.backend.core.corpus import (  # noqa: E402
    INDEX_FILE,
    chunk_document,
    clean_wiki_text,
    is_usable,
...
from apps.backend.core.kg import extract_names  # noqa: E402
from apps.backend.core.nerlabel import allowed_names, n_labels, ner_label  # noqa: E402

# SYSTEM prompt và cách ghép user message ĐẶT MỘT CHỖ DUY NHẤT (apps/backend/core/prompt.py)
# và được import ở cả train, serve (llm.py) và eval (score_gold.py). Bản cũ copy
# chuỗi này ở 3 file với 3 nội dung khác nhau.
from apps.backend.core.prompt import NER_QUESTION, NER_TYPES, SYSTEM, user_msg  # noqa: E402
...
def make_ner_samples(rng: random.Random, docs: list[dict], per_doc: int = NER_PER_DOC) -> list[dict]:
    """Mẫu NER với nhãn vàng lấy từ chính graph deterministic (apps/backend/core/nerlabel.py).
...
def main() -> None:
    ...
    if not INDEX_FILE.exists():
        print(f"ERROR: chưa có {INDEX_FILE}. Chạy pipelines/ingestion/crawl_by_location.py trước.")
        sys.exit(1)

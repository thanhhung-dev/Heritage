...
VÌ SAO CẦN FILE NÀY
  ...
  2. bootstrap_v3.py ghi NER ra file RIÊNG (ner_train_v3.jsonl) để đo copy-rate
     ...
     tách file KHÔNG có nghĩa là loại khỏi training: apps/backend/core/llm.py serve
     ...
     pipelines/evaluation/report_base.json). Ở đây gộp lại, có kiểm soát tỉ lệ.

Chạy:
  apps/backend/.venv/bin/python pipelines/training/make_train_set.py            # gộp + sao lưu
  apps/backend/.venv/bin/python pipelines/training/make_train_set.py --ner-ratio 0.15
  apps/backend/.venv/bin/python pipelines/training/make_train_set.py --dry-run   # chỉ xem
"""
...
from pipelines.training.bootstrap_deep_qa import SEED  # noqa: E402
...
def read_jsonl(path: Path) -> list[dict]:
    if not path.exists():
        print(f"ERROR: thiếu {path}. Chạy pipelines/training/bootstrap_v3.py trước.")
        sys.exit(1)
...
def main() -> None:
    ...
    print(f"\n  Đặt trong pipelines/training/lora_config.yaml: iters: {iters}   "
          f"(batch_size 2, grad_accum 4 -> ~3 epoch trên {n_train} mẫu)")
    print("  Rồi: bash pipelines/training/train.sh --fresh")

...
from pipelines.training.train_hf import find_last_checkpoint, load_config, tokenize_conversation
...
class TrainHfTest(unittest.TestCase):
    ...
    def test_default_workflow_is_lora_without_four_bit_quantization(self):
        config = load_config(Path("pipelines/training/lora_config.yaml"))
        requirements = Path("pipelines/training/requirements.txt").read_text(encoding="utf-8")
